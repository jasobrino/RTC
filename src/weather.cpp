...
const char* API_openweather = "http://api.openweathermap.org/data/2.5/weather?id=2512989&units=metric&lang=es&appid=db6d40a9824fd954f79179f890878d3d";
const char* API_openweather_onecall = "http://api.openweathermap.org/data/2.5/onecall?lat=39.5694&lon=2.6502&units=metric&lang=es&exclude=minutely,hourly&appid=db6d40a9824fd954f79179f890878d3d";
...
// captura los datos de openweather y los guarda en la estructura datos
// bool getOpenweatherData() { 
//     if(WiFi.status() != WL_CONNECTED) {
//        if(wifi_init() != WL_CONNECTED) {
//            Serial.println("getOpenweatherData: sin acceso a wifi");
//            return false;
//        }
//     }
//     if( !http.begin(client, API_openweather) ) { 
//         Serial.printf("HTTP no ha sido posible conectar\n");
//         return false;
//     }
...
//         StaticJsonDocument<200> filtro;
//         filtro["weather"][0] = true;
//         filtro["main"]["temp"] = true;
//         filtro["main"]["temp_min"] = true;
//         filtro["main"]["temp_max"] = true;
//         filtro["main"]["humidity"] = true;
//         filtro["dt"] = true;
//         filtro["cod"] = true;
//         filtro["sys"]["sunset"] = true;
//         filtro["sys"]["sunrise"] = true;
...
//         if(error) {
//             Serial.print(F("error en DeserializeJson (datos): "));
//             Serial.println(error.f_str());
//             return false;
//         } 
//         http.end();
//         weather_data.description = doc["weather"][0]["description"].as<String>();
//         weather_data.icon = doc["weather"][0]["icon"].as<String>();
//         weather_data.dt = spainTZ.toLocal(doc["dt"]);
//         weather_data.temp = doc["main"]["temp"];
//         weather_data.temp_min = doc["main"]["temp_min"];
//         weather_data.temp_max = doc["main"]["temp_max"];
//         weather_data.humidity = doc["main"]["humidity"];
//         weather_data.sunset = spainTZ.toLocal(doc["sys"]["sunset"]);
//         weather_data.sunrise = spainTZ.toLocal(doc["sys"]["sunrise"]);
//         return true;
//     } else {
...
bool getOpenweatherData() { 
    ...
    if( !http.begin(client, API_openweather_onecall) ) { 
